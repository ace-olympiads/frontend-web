...
export async function getServerSideProps(context: any) {
  ...
  try {
    const response = await fetch(`${process.env.BACKEND_URL || "http://localhost:8000/"}question/examination/${id}`);
    const questions = await response.json();
    const examResponse = await fetch(`${process.env.BACKEND_URL || "http://localhost:8000/"}question/examinations/`);
